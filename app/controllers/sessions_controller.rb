class SessionsController < ApplicationController
  ...
  def create
    ...
    user_resp = Faraday.get "https://api.github.com/user", {}, {"Authorization" => "token #{session[:token]}"}
    u_resp_body = JSON.parse(user_resp.body)
    session[:username] = u_resp_body["login"]
